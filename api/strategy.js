...
module.exports = async function handler(req, res) {
  ...
  // GET /api/strategy?workspace_id=xxx
  if (req.method === 'GET') {
  ...
  // POST /api/strategy — save strategy data
  if (req.method === 'POST') {
    const body = req.body || {};
    ...
    await sql`
      INSERT INTO strategy_data (workspace_id, data, updated_at)
      VALUES (${wid}, ${dataStr}, ${now})
      ON CONFLICT (workspace_id)
      DO UPDATE SET data = EXCLUDED.data, updated_at = EXCLUDED.updated_at
    `;

    await sql`UPDATE workspaces SET runs_count = runs_count + 1 WHERE id = ${wid}`;
